...
public class AI_TurretController : MonoBehaviour
{
    ...
    public bool TrySwitchShell(ShellType type)
    {
        if (isReloading || isSwitchingShell || currentShell == type)
            return false;
   ...
   IEnumerator RecoilCannon()//moves cannon backwords and the forward to simulate recoil when shooting, its a coroutine so it runs over time and not instant
   {
    ...
    public bool TrySwitchMode(FireMode targetMode)
    {
        if (isSwitchingMode || activeMode == targetMode)
            return false;

        StartCoroutine(SwitchFireMode(targetMode));
        return true;
    }
  ...
  void Shoot()
{
    ...
    IEnumerator SwitchFireMode(FireMode targetMode)
    {
